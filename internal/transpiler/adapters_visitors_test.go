...
func (s *stubAVisitor) VisitList(ctx *parser.ListContext) (a.Value, error)      { return a.NewBasicValue("L", "list"), nil }
func (s *stubAVisitor) VisitArray(ctx *parser.ArrayContext) (a.Value, error)    { return a.NewBasicValue("A", "array"), nil }
func (s *stubAVisitor) VisitTerminal(node antlr.TerminalNode) (a.Value, error)  { return a.NewBasicValue(node.GetText(), "symbol"), nil }
...
func (s *stubCGVisitor) VisitList(ctx *parser.ListContext) (cg.Value, error)      { return a.NewBasicValue("L", "list"), nil }
func (s *stubCGVisitor) VisitArray(ctx *parser.ArrayContext) (cg.Value, error)    { return a.NewBasicValue("A", "array"), nil }
func (s *stubCGVisitor) VisitTerminal(node antlr.TerminalNode) (cg.Value, error)  { return a.NewBasicValue(node.GetText(), "symbol"), nil }
...
func (s *stubMacroVisitor) VisitList(ctx *parser.ListContext) (macro.Value, error)   { return a.NewBasicValue("ML", "list"), nil }
func (s *stubMacroVisitor) VisitArray(ctx *parser.ArrayContext) (macro.Value, error) { return a.NewBasicValue("MA", "array"), nil }
func (s *stubMacroVisitor) VisitTerminal(node antlr.TerminalNode) (macro.Value, error) { return a.NewBasicValue(node.GetText(), "symbol"), nil }
...
func TestAnalysisVisitorBridge_Methods(t *testing.T) {
    ...
    arr := arrParser.Array()
    if arr == nil { t.Fatalf("expected array parse") }
...
func TestMacroVisitorBridge_Methods(t *testing.T) {
    ...
    if list, ok := prog.GetChild(0).(*parser.ListContext); ok {
        if v, err := br.VisitList(list); err != nil || v == nil || v.String() != "ML" { t.Fatalf("VisitList: %v %v", err, v) }
        if v, _ := br.VisitList(list); v == nil || v.Type() != "list" { t.Fatalf("VisitList Type(): %v", v) }
    }
    arrInput := antlr.NewInputStream("[1 2]")
    arrLex := parser.NewVexLexer(arrInput)
    arrTs := antlr.NewCommonTokenStream(arrLex, 0)
    ...
    if arr := arrParser.Array(); arr != nil {
        if v, err := br.VisitArray(arr.(*parser.ArrayContext)); err != nil || v == nil || v.String() != "MA" { t.Fatalf("VisitArray: %v %v", err, v) }
        if v, _ := br.VisitArray(arr.(*parser.ArrayContext)); v == nil || v.Type() != "array" { t.Fatalf("VisitArray Type(): %v", v) }
    }
    tlex := parser.NewVexLexer(antlr.NewInputStream("z"))
    tts := antlr.NewCommonTokenStream(tlex, 0)
    ttok := tts.Get(0)
    term := antlr.NewTerminalNodeImpl(ttok)
    if v, err := br.VisitTerminal(term); err != nil || v == nil || v.String() != "z" { t.Fatalf("VisitTerminal: %v %v", err, v) }
...
func TestMacroExpanderAdapter_RegisterHasGet(t *testing.T) {
    ...
    if err := mea.RegisterMacro("m", m); err != nil { t.Fatalf("register: %v", err) }
    if !mea.HasMacro("m") { t.Fatalf("expected HasMacro true") }
