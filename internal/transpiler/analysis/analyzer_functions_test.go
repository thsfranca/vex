...
func TestParseSingleExpression_Variants(t *testing.T) {
	a := NewAnalyzer()
	// List
	if n := a.parseSingleExpression("(+ 1 2)"); n == nil {
		t.Fatalf("list parse returned nil")
	}
	// Program fallback
	if n := a.parseSingleExpression("(+ 1 2) (+ 3 4)"); n == nil {
		t.Fatalf("program parse returned nil")
	}
}
...
func TestAnalyzeFunctionCall_Branches(t *testing.T) {
	...
	if v, err := a.VisitList(p.List().(*parser.ListContext)); err != nil || v == nil {
		t.Fatalf("+ path failed: %v %#v", err, v)
	}
	// unary '-' path
	...
	if v, err := a.VisitList(p2.List().(*parser.ListContext)); err != nil || v == nil {
		t.Fatalf("- path failed: %v %#v", err, v)
	}
...
func TestTypeHelpers_IntFloat(t *testing.T) {
	if !isInt("42") || isInt("42.0") {
		t.Fatalf("isInt checks failed")
	}
	if !isFloat("1.2") || isFloat("1.") || isFloat(".2") {
		t.Fatalf("isFloat checks failed")
	}
}
...
func TestVisitNode_DefaultUnknown(t *testing.T) {
	...
	if v, err := a.visitNode(&unknownTree{}); err == nil || v == nil || v.Type() != "undefined" {
		t.Fatalf("expected unknown handling")
	}
}
