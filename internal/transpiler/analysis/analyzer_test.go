...
func TestAnalyzer_VisitTerminal_UndefinedSymbol(t *testing.T) {
	...
	hasCode := false
	for _, e := range errs {
		if strings.Contains(e.Message, "TYPE-UNDEFINED") {
			hasCode = true
			break
		}
	}
	if !hasCode {
		t.Fatalf("expected TYPE-UNDEFINED diagnostic, got: %s", analyzer.errorReporter.FormatErrors())
	}
}
...
func TestAnalyzer_ExplicitTypeFunctionDefinition(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("analyzeDef failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("VisitTerminal failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("function call with correct type should succeed: %v", err)
	}
...
func TestAnalyzer_ArrayPrimitivesTyping(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get typing failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("slice typing failed: %v", err)
	}
	if got2.Type() != "[]interface{}" {
		t.Fatalf("slice type = %s, want []interface{}", got2.Type())
	}
	...
	if err != nil {
		t.Fatalf("append typing failed: %v", err)
	}
	if got3.Type() != "[]interface{}" {
		t.Fatalf("append type = %s, want []interface{}", got3.Type())
	}
}
...
func TestEqualityTypingViaScheme(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("equality int==int should type: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("equality string==string should type: %v", err)
	}
}
...
func TestIfConditionRequiresBool_Negative(t *testing.T) {
	...
	ok := false
	for _, e := range errs {
		if strings.Contains(e.Message, "TYPE-CONDITION") {
			ok = true
			break
		}
	}
	if !ok {
		t.Fatalf("expected TYPE-CONDITION diagnostic")
	}
}
...
func TestAnalyzer_ValueRestriction_GeneralizeOnlyValues(t *testing.T) {
	...
	if _, ok := a.typeEnv["id"]; !ok {
		t.Fatalf("id should be generalized")
	}
...
func (m *mockTerminalNode) ToStringTree(ruleNames []string, recog antlr.Recognizer) string {
	return m.text
}
