...
func TestGenerator_extractImportPair_ListAndArray(t *testing.T) {
	...
	if p != "fmt" || a != "alias" {
		t.Fatalf("list pair mismatch: %q %q", p, a)
	}
	// Array form: ["os" a]
	...
	if p2 != "os" || a2 != "a" {
		t.Fatalf("array pair mismatch: %q %q", p2, a2)
	}
}
...
func TestGenerator_visitNode_DefaultAndNodeToString(t *testing.T) {
	...
	if err != nil || v == nil || v.String() == "" {
		t.Fatalf("visitNode default failed: %v %#v", err, v)
	}
	...
	if s := g.nodeToString(list); s == "" {
		t.Fatalf("nodeToString empty for rule node")
	}
}
