...
func TestResolver_PkgSchemes_CrossPackageIntegration(t *testing.T) {
	dir := t.TempDir()
	// Package a exports simple function and constant n
	writeFile(t, filepath.Join(dir, "a", "a.vx"), "(import \"vex.core\")\n(export [simple-func n])\n(def simple-func (fn [x: int] -> int x))\n(def n 42)\n")
	// Entry imports a and calls a/simple-func
	entry := filepath.Join(dir, "main.vx")
	writeFile(t, entry, "(import \"vex.core\")\n(import [\"a\"])\n(def main (do (a/simple-func 1) a/n))\n")
	...
	if _, ok := res.PkgSchemes["a"]["simple-func"]; !ok {
		t.Fatalf("expected scheme for a/simple-func to be collected")
	}
}
