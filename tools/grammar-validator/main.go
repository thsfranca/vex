...
func main() {
	...
	if !filesExist("parser/vex_lexer.go", "parser/vex_parser.go") {
		fmt.Fprintf(os.Stderr, "[ERROR] Generated Vex parser files not found.\n")
	...
	for _, filename := range files {
		...
		if errorListener.hasError {
			if mode == "invalid" {
				fmt.Printf("[SUCCESS] %s failed as expected (invalid test case)\n", filename)
			} else {
				fmt.Printf("[ERROR] %s failed to parse\n", filename)
				failedFiles = append(failedFiles, filename)
			}
		} else {
			if mode == "valid" || mode == "default" {
				fmt.Printf("[SUCCESS] %s parsed successfully\n", filename)
				fmt.Printf("Parse tree: %s\n", tree.ToStringTree(vexParser.GetRuleNames(), vexParser))
			} else { // mode == "invalid"
				fmt.Printf("[ERROR] %s parsed successfully but should have failed (invalid test case)\n", filename)
	...
	if hasErrors {
		...
		switch mode {
		case "valid":
			fmt.Println("\n[SUCCESS] All valid example files parsed successfully!")
		case "invalid":
			fmt.Println("\n[SUCCESS] All invalid example files failed as expected!")
		default:
			fmt.Println("\n[SUCCESS] All example files produced expected results!")
		}
	}
}
